...
- **Two credentials are accepted while the client migrates.** `AuthTokenFilter` (order 5) reads `Authorization: Bearer` and runs ahead of `ApiKeyFilter` (order 10), publishing the *same* `userId` attribute — which is why no controller, service or repository changed to gain token auth. A request with no bearer header falls through to the key; a request with a **bad** one is refused there and does not fall through, because otherwise the outcome would depend on which credential was checked first and a stale token would be silently rescued by a key the client still carried. `ApiKeyFilter.shouldNotFilter` skips a request whose owner attribute is already set, expressed as a property of the request rather than a reference to the other filter, so deleting either leaves the other correct.
- **`PublicRoutes` is the single answer to what is reachable unauthenticated**, consulted by both filters so a route cannot be open in one and closed in the other. The real control is `PublicRoutesTest`, which enumerates the **actual mapped handler routes** out of the running context and asserts the public set exactly — a prefix exemption is convenient precisely because a new route can join it without anyone deciding it should. That test is how `/error` was found: Boot declares it, this application cannot remove it, so it is listed deliberately and a sibling test pins that reaching it directly discloses no message, trace or class name.
- **Tokens are digested, passphrases are hashed, and the difference is the design.** `auth_token` stores SHA-256 of a 32-byte `SecureRandom` token — no low-entropy guess to slow down, and bcrypt there would charge its cost to every authenticated request. The passphrase goes through bcrypt because its input *is* low-entropy. `spring-security-crypto` is the standalone artifact, **not** `spring-boot-starter-security`, which would bring a filter chain to switch off; it resolves through the Boot 4 BOM at 7.1.0 with no transitive dependencies. Hashes carry their own cost factor, so `PassphraseHashTool` writes at 12, tests write at 4, and the server verifies whatever it is given without being told.
- **Rotation is what makes a thirty-day refresh token affordable**, and `rotated_at` is deliberately not `revoked_at`: revoked means somebody withdrew it, rotated means it was spent. Only the second makes a *later* presentation evidence that two parties hold the token, which is the entire basis of reuse detection — so detecting it revokes the whole `family_id`, access token included, and sends the legitimate client back to the passphrase too. That is the cost of not being able to tell the two holders apart, and much the cheaper failure. Logout revokes the family and **only** the family, so signing out on one device leaves another alone.
- **`TokenService.refresh` is `@Transactional(noRollbackFor = AuthenticationFailedException.class)` and that is load-bearing.** Reuse detection writes the revocation and *then* refuses the request; a `RuntimeException` rolls back by default, which would undo exactly the revocation that is the whole response to a stolen token — while the caller still saw a `401`, so nothing would look wrong and the copied token would keep working for a month. `RefreshTokenTest.reuseRevokesTheFamily` caught it; it was written before the annotation existed and failed.
- **A malformed passphrase hash counts as absent, not as configured.** `PassphraseAuthenticator.configured()` checks the value *looks* like bcrypt, mirroring `BCryptPasswordEncoder`'s own pattern so the two cannot disagree. This is not defensive padding — it cost two rounds of debugging against the live deployment. A hash whose `$` sequences are eaten on the way into an environment variable (what a shell or dotenv parser does to `$2a$12$…`) is still non-blank, so the old check called sign-in available and the endpoint answered `401` to the *correct* passphrase: a server fault reported as the caller's, and indistinguishable from a wrong guess. It now reads as unconfigured, warns once at startup naming the likely cause, and spends none of the caller's allowance because the configured check runs ahead of the rate limit.
- **Sign-in is optional until the key is removed.** `flashcards.passphrase-hash` is nullable and `POST /auth/login` answers `503` when it is unset — the `GeminiProperties` posture, so landing this does not refuse to start a deployment that has not configured one yet. It becomes `@NotBlank` in the change that deletes the key.
...
- **Sign-in is rationed too, and for a different reason than generation.** `POST /auth/login` is the only route an unauthenticated caller can put a body into and it deliberately runs bcrypt, so it is both a place to guess a passphrase and a place to spend a small instance's CPU. `LoginRateLimit` counts **failures only** — counting successes would eventually lock somebody out for using their own account — over a **rolling** fifteen-minute window, since a fixed one hands out double the allowance across its boundary. The check runs **before** the passphrase is hashed, which is why the correct passphrase is refused too while the limit holds: deciding otherwise would mean doing the expensive work the limit exists to avoid.
- **It keeps a second, global count, and that is the honest half.** The per-address limit keys on `getRemoteAddr()`, which behind a proxy comes from `X-Forwarded-For` and can be varied per attempt — so on its own it would offer assurance it does not provide. `MAX_GLOBAL` cannot be evaded that way and is deliberately neither documented nor reported. `server.forward-headers-strategy=framework` is what makes the per-address half work at all, and it is sound **only** because the app is reachable solely through the platform's router; that stops being true if `FLASHCARDS_BIND_ADDRESS` opens it to a network with no proxy in front.
- **`LoginRateLimit` is its own bean for the same transactional reason `GenerationQuota` is.** It records a failure and the controller then throws, and in one transaction that rollback would erase the record the limit counts — every response would still look correct while an attacker got unlimited attempts. This is the second time that trap has appeared in this feature; `TokenService.refresh` needed `noRollbackFor` for the same underlying reason.
